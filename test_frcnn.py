...
def predict_single_image(img_path, model_rpn, model_classifier, cfg, class_mapping):
    ...
    for jk in range(result.shape[0] // cfg.num_rois + 1):
        ...
        # 首先判断类是否是前景，再根据所属类别得到roi之前的边界大小
    ...
    img = draw_boxes_and_label_on_image_cv2(img, class_mapping, rpn_boxes)
...
def parse_args():
    ...
    parser.add_argument('--path', '-p', default='./images/', help='val image path')
